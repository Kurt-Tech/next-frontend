  ...
  return (
    <header className="sticky top-0 z-50 border-b border-border/60 bg-background/80 backdrop-blur">
      <div className="mx-auto flex h-16 w-full max-w-6xl items-center justify-between px-0 sm:px-6">
        <Link href="/" className="relative flex h-10 w-32 items-center md:w-40" aria-label="Apex Medical Group">
          <Image src={LOGO_SRC} alt="Apex Medical Group" fill priority sizes="(max-width: 768px) 128px, 160px" className="object-contain" />
        </Link>
      ...
      <Dialog.Portal>
        ...
        <Dialog.Content className="fixed inset-y-0 right-0 z-50 flex w-72 max-w-full flex-col gap-8 bg-background px-6 pb-8 pt-6 shadow-2xl md:hidden">
          <div className="flex items-center justify-between">
            <Link href="/" className="relative flex h-9 w-28 items-center" aria-label="Apex Medical Group">
              <Image src={LOGO_SRC} alt="Apex Medical Group" fill sizes="112px" className="object-contain" />
            </Link>
